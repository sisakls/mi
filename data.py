...
@gin.configurable
class Data:
    def __init__(self, dataset_name, dim, alpha=1., noise_dim=0, seed=0):
    ...
    def get_dset_linear(self):
        self.var_z = self.rng.uniform(-self.alpha/2, self.alpha/2, size=self.dim)
        self.var_x = self.rng.uniform(0, 1, size=self.dim)
    ...
    def add_noise(self):
        ...
        self.var_x = np.concatenate(
            [self.var_x, self.rng.normal(0, 1, size=self.noise_dim)]) #noise dimensions
        self.var_y = np.concatenate(
            [self.var_y, self.rng.normal(0, 1, size=self.noise_dim)])